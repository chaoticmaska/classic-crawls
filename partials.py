import argparse
from utils.partialsData import mapCrawlArgs, availableEncounters, mapRaidZone
...
class Report:
    ...
    def getUptime(self):
        ...
        response = reportDebuffsQuery(
            self.url,
            self.enemyID,
            encounterData.get('curseID'),
            encounterData.get('damageModifiers')
                ...
                cEvent = next(
                    filter(
                        lambda e: e.get('timestamp') >= event.get('timestamp') and
                                  ...
                                  e.get('abilityGameID') == event.get('abilityGameID'),
                        damage
                    )
                )
...
if __name__ == '__main__':  # General stuff like parse args and run crawls for selected zones
    ...
    for zone in z:
        ...
        if zone == 'AQ':
            ...
            for encounter in mapRaidZone(1000):
                encounterData = mapCrawlArgs(s, encounter.get('name'))
                run()
        elif zone == 'BWL':
            for encounter in mapRaidZone(1002):
                encounterData = mapCrawlArgs(s, encounter.get('name'))
                run()
        # Single encounter or trash mob
        else:
